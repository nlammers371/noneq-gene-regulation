...
%         specificityFactor = log10((right_weights./wrong_weights)./simInfo.specFactor.*(cwVec.^max_r)); 
        
        right_weights = sum(simInfo.n_right_bound.*state_probs_cs,2);
        wrong_weights = sum(simInfo.n_wrong_bound.*state_probs_cs,2);
        specificityFactor = log10((right_weights./wrong_weights)./simInfo.specFactor.*(cwVec).*2); 
    else
