...
simInfo.a1 = a1;
simInfo.a0 = a0;
simInfo.calculateRateEntropy = calculateRateEntropy;
...
parfor nti = 1:n_sim%(nti = 1:n_sim,useParpool*NumWorkers) %NL: note that if useParpool=0, this will switch to serial execution for % %% %%
    ...
    simResults(nti).sim_id_vec = nti*ones(size(iter_id_vec(1:last_index,:)));
    simResults(nti).area_vec = area_vec(1:i_pass-1);
    simResults(nti).area_id_vec = nti*ones(size(area_vec(1:i_pass-1)));
