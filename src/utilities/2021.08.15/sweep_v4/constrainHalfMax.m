function [rates_out,error_flags] = constrainHalfMax(rate_array,c_range,rate_bounds)
...
ref_rates = cat(3,ref_rates1,ref_rates2);%ref_rates = cat(3,ref_rates1(lin_ind_array),ref_rates2(lin_ind_array));
...
%% 
nOptions = sum(valid_bin_array,2);
choice_array = cumsum(valid_bin_array,2)./nOptions > rand(size(nOptions));
[m_vals, m_ids] = max(choice_array,[],2);

%%
% sample
% [m_vals,m_ids] = nanmax(nanmin(valid_bin_array,[],3),[],2);
lin_index_sub = sub2ind(size(ref_array),1:size(ref_array,1),m_ids');
col_index = mod(m_ids-1,3)+1;%ref_array(lin_index_sub);
...
ProductionRate = fourStateProduction_v4(rates_out,1);
err_flags2 = round(ProductionRate,2)~=0.5 & ~error_flags;
rates_out(err_flags2,:) = NaN;
