function CycleTime = fourStateCycleTimeGeneral(rate_vec,c_range)
...
CycleTime = ...
   ((r1.*r2+k3.*(k4+r2)).*(k2+r3)+(r1.*r2+(k4+r1+r2).*r3).*r4+k1.*( ...
  k3.*k4+k2.*(k3+k4+r1)+(k4+r1).*r4)).^(-1).*((k1.*(k4+r1)+r1.*r2) ...
  .^(-1).*(k3.*(k1.^2.*k2+(k4.*(k4+r1)+2.*k4.*r2+r2.^2).*(k2+r3)+ ...
  k1.*(k2.*(k4+2.*r2)+r2.*r3))+((k4+r1).^2+(k1+2.*k4+r1).*r2+r2.^2) ...
  .*r3.*r4)+(k2.*k3+r3.*(k3+r4)).^(-1).*(r1.*r2.*((k2+r3).^2+(2.*k2+ ...
  k3+r3).*r4+r4.^2)+k1.*(k2.^2.*(k4+r1)+(k3+r4).*(k3.*k4+(k4+r1).* ...
  r4)+k2.*(k3.*k4+(k4+r1).*(r3+2.*r4)))));
