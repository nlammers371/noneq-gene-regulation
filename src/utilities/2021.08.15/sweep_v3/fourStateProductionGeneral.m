function productionRate = fourStateProductionGeneral(rate_vec,c_range)
...
productionRate = ...
(c_range.*k2.*k3.*(k1+k4+c_range.*r2)+k3.*(k4+c_range.*r2).*r3+(k4+r1+c_range.*r2).*r3.* ...
  r4).*(c_range.^2.*k2.*(k3+r1).*r2+(k1+r3).*(k3.*k4+(k4+r1).*r4)+c_range.*(k2.* ...
