function assymptoticVariance = fourStateVariance_v2(rate_vec,c_range,repressor_flag)
...
assymptoticVariance = ...
    2.*(k1.*k2.*k3+k2.*k3.*r2+r2.*r3.*(k3+r4)).*((r1.*r2+k3.*(k4+r2)) ...
      .*(k2+r3)+(r1.*r2+(k4+r1+r2).*r3).*r4+k1.*(k3.*k4+k2.*(k3+k4+r1)+( ...
      k4+r1).*r4)).^(-3).*((k1.*k2.*(k4+r1)+r1.*r2.*(k2+r3)).*(r2.*(k2+ ...
      r3)+(r2+r3).*r4+k1.*(k2+r4))+(k3.*k4.*(k2+r3)+(k4+r1).*r3.*r4).*( ...
