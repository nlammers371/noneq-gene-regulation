...
function [networkInfo, RSymSmall, RSym] = makeActivatorGeneralNetworkNSWrong(networkInfo6,nSpecSites,nNSSites,nGeneralReactions)
    ...
    n_right_bound_ns = zeros(size(n_right_bound_init));
    n_wrong_bound = n_wrong_bound_init;
    n_wrong_bound_ns = zeros(size(n_right_bound_init));
    ...
    % first block  
    nSites = nSpecSites + nNSSites;
    
    % add binding reactions first
    for n = 1:nSpecSites-1
        ...
        n_right_bound_ns = repmat(n_right_bound_ns,1,3);
        n_wrong_bound_ns = repmat(n_wrong_bound_ns,1,3);
    ...
    for n = 1:nNSSites
        ...
        % double and increment binding info vec
        n_right_bound_ns = repmat(n_right_bound_ns,1,3);   
        n_right_bound_ns(baseNew+1:2*baseNew) = n_right_bound_ns(baseNew+1:2*baseNew) + 1;
                 
        n_wrong_bound_ns = repmat(n_wrong_bound_ns,1,3);
        n_wrong_bound_ns(2*baseNew+1:3*baseNew) = n_wrong_bound_ns(2*baseNew+1:3*baseNew) + 1;
        ...
        for i = 1:baseNew
            ...
            for j = 1:2
                ...
                % add binding info
                transitionInfoArray(ind2,ind1) = binding_ids(j); 
                transitionInfoArray(ind1,ind2) = -2; % all unbinding reactions are non-specific
            end
        end
    end
    ...
    for n = 1:nGeneralReactions-1
        ...
        n_right_bound_ns = repmat(n_right_bound_ns,1,2);   
        n_wrong_bound_ns = repmat(n_wrong_bound_ns,1,2);  
    ...
    n_total_ns = n_right_bound_ns + n_wrong_bound_ns; % right
    
    full_state_array = [n_g_engaged' n_right_bound' n_wrong_bound' n_total_ns'];
    ...
    networkInfo.n_right_bound_ns = n_right_bound_ns(ia);  
    networkInfo.n_right_bound_full_ns = n_right_bound_ns;
    networkInfo.n_wrong_bound_ns = n_wrong_bound_ns(ia);  
    networkInfo.n_wrong_bound_full_ns = n_wrong_bound_ns;
