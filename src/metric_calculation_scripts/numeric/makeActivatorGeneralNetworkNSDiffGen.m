...
function [networkInfo, RSymSmall, RSym] = makeActivatorGeneralNetworkNSDiffGen(networkInfo6,nSites,nGeneralReactions)
...
%     diff_array(~unbinding_reactions) = 0;
%     diff_array(unbinding_reactions) = round(diff_array(unbinding_reactions) - 0.1,2);
    ...
    for j = 1:nGeneralReactions
        ...
        % add to RSym
        % g->a
        RSym(binding_reactions & general_id_array(j,:)) = RSym(binding_reactions & general_id_array(j,:))*sym_vec(end-3);
        RSym(unbinding_reactions & general_id_array(j,:)) = RSym(unbinding_reactions & general_id_array(j,:))*sym_vec(end-2);

        % a->g
        for n = 1:nSites%             
            RSym(transitionInfoArray==3+(j-1)/10&n_total_bound==n) = RSym(transitionInfoArray==3+(j-1)/10&n_total_bound==n)*sym_vec(end-1)^n;
            RSym(transitionInfoArray==-3-(j-1)/10&n_total_bound==n) = RSym(transitionInfoArray==-3-(j-1)/10&n_total_bound==n)*sym_vec(end)^n;
        end        
    ...
    full_state_array = [general_id_array' n_right_bound' n_wrong_bound'];
