function [networkInfo, RSym] = addSpecBindingSite_v2(networkInfoInit)
...
%         d = sym(['kmi' num2str(nSites)],'positive');
    ...
    networkInfo.sweepVarList = [networkInfo.sweepVarList a b];
    networkInfo.sweepVarStrings = cellstr(string(networkInfo.sweepVarList));
    networkInfo.defaultValues = [networkInfo.defaultValues 1 1];
    networkInfo.sweepFlags = [networkInfo.sweepFlags true(1,2)];
    networkInfo.fourStateInputFlags = [networkInfo.fourStateInputFlags false(1,2)];
...
%     bkd_prev_sym = sym(['kmi' num2str(nSites-1)],'positive');   
    ...
    % initialize weights to allow for impact of 2 bound
    syms kip2 kap2 wma wmi positive
    
    % Add info for equilibrium constraint application and cycle flux
    % calculations    
    networkInfo.forwardRateConstants(end+1) = {[wma a]};
    networkInfo.forwardRateAdjustFlags(end+1) = {[0 1]==1};
    networkInfo.forwardRateIndices(end+1) = {find(ismember(networkInfo.sweepVarList,networkInfo.forwardRateConstants{end}))};
    networkInfo.backwardRateConstants(end+1) = {[wmi b]};
    networkInfo.backwardRateAdjustFlags(end+1) = {[0 1]==1};
