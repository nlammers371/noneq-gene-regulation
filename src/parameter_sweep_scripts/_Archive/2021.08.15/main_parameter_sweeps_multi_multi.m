...
nStateArray = (n_bs_vec+1).*(n_g_vec+1)';
nStateArray(:,:,2) = (n_bs_cw).*(n_g_vec+1)';
...
for i = 1:length(ns_flag_vec)
    for m = 1:length(n_g_vec)
        for n = 1:length(n_bs_vec) 
            ...
            functionPath = [writePath 'n' sprintf('%03d',nStates) '_s' sprintf('%02d',n_bs_vec(n)) '_ns00_g' sprintf('%02d',n_g_vec(m)) '_cw' num2str(ns_flag_vec(i)) filesep];
...
iter = 1;
for m = 1:3%:length(n_g_vec)
    for n = 2%length(n_bs_vec) 
        ...
        [results_struct(iter).sim_info, results_struct(iter).sim_struct] = ...
                            param_sweep_multi_v3([phi_index sharpness_index],...
                            functionPathCell{m,n}, sweep_options{:},...
                            'half_max_flag',false,'equilibrium_flag',false,'nStates',nStateArray(n),'TauCycleTime',3600);
        toc;    
        iter = iter + 1;
    end
end    
...
figure;
hold on
for n = 3:-1:1%length(nStateVec):-1:1
...
%     scatter(exp(metric_array_long(:,phi_index)),metric_array_long(:,ir_index))    
    scatter(exp(metric_array_long(phi_filter&tau_filter,phi_index)),metric_array_long(phi_filter&tau_filter,sharpness_index))    
end
xlim([1e-4 1e4])
% ylim([0 0.03])
set(gca,'xscale','log');
...
[ir_max,ir_i] = nanmax(results_struct(n).metric_array_long(:,sharpness_index).*tau_multiplier.*phi_multiplier)
