function [simulation_results, fpath] = param_sweep_v2(metric_indices,varargin)
addpath('../utilities')
outPath = '../../out/bivariate_parameter_sweeps_v2/';
mkdir(outPath)
...
% metric bounds
metric_ub_vec = [Inf Inf 15 Inf Inf Inf];
metric_lb_vec = [-Inf -Inf -15 0 0 0];
...
simulation_results = struct;
    ...
    simulation_results(mdi).c_val = 1; 
    simulation_results(mdi).rate_bounds = rate_bounds;     
    simulation_results(mdi).repressor_flag = repressor_flag;     
            ...
            for b = 1:n_bound_samples
                for s = 1:n_seeds
                    ...
                    while ~pass_flag
                        ...
                        % calculate  metrics
                        metric_vec = calculateMetricsV3(orig_rate_vec,repressor_flag,h_max_flag);                                        
            ...
            for b = 1:n_bound_samples
                ...
                for k = 1:n_seeds          
                    ...
                    while ~pass_flag
                        ...
                        metric_vec = calculateMetricsV3(new_rate_vec,repressor_flag,h_max_flag);       
                        pass_flag = all(metric_vec>=metric_lb_vec)&all(metric_vec<=metric_ub_vec);
                    end
                    % record                   
    ...
    simulation_results(mdi).metric_array = metric_array_out;        
    simulation_results(mdi).rate_array = rate_array_out;    
end
toc
% save
