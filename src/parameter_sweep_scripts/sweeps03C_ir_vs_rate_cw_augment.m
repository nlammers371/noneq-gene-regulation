% script to call augment IR vs. w sweep results for case when N_LC = 5 and
% w <~ 10
...
writePath = [DropboxFolder filesep 'sweeps03C_ir_vs_rate_cw_re-run' filesep];
mkdir(writePath);
...
lc_vec = [1];
eq_vec = [0];
cw_vec = 100;%unique(round(logspace(0,2,25)));%[1 10 1e2];
...
%     end            
    ...
    suffix = [suffix '_w' sprintf('%04d',round(cw))];
    % save
    disp('saving...')
