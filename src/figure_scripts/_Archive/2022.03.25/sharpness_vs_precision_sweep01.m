...
bound_indices_neq = randsample(find(plot_filter_neq),n_samp,true);
plot_indices_neq = randsample(find(plot_filter_neq),n_plot,true);

% find neq boundary
b_points_neq = boundary(sharpness_vec_neq(bound_indices_neq),precision_vec_neq(bound_indices_neq),1);
%%%%%%%%%%%%%
% eq
...
bound_indices_eq = randsample(find(plot_filter_eq),n_samp,true);
plot_indices_eq = randsample(find(plot_filter_eq),n_plot,true);
% find eq boundary
b_points_eq = boundary(sharpness_vec_eq(bound_indices_eq),precision_vec_eq(bound_indices_eq),1);
...
% ir_99 = 0.99*nanmax(ir_vec_neq);
ir_options = find(ir_vec_neq>=ir_99&sharpness_vec_neq>=0);
...
sharp_v_prec_4 = figure;
cmap = brewermap(8,'Set2');

% plot eq and neq domains
hold on
fill(sharpness_vec_neq(bound_indices_neq(b_points_neq)),precision_vec_neq(bound_indices_neq(b_points_neq)),...
            cmap(2,:),'EdgeColor',brighten(cmap(2,:),-0.5),'LineWidth',3,'FaceAlpha',0.6)
fill(sharpness_vec_eq(bound_indices_eq(b_points_eq)),precision_vec_eq(bound_indices_eq(b_points_eq)),...
            cmap(3,:),'EdgeColor',brighten(cmap(3,:),-0.5),'LineWidth',3,'FaceAlpha',1)  
...
sharp_v_prec_4.InvertHardcopy = 'off';
set(gcf,'color','w');

ylim([0.25 1])

saveas(sharp_v_prec_4,[FigPath 'sharp_vs_prec4.png'])
saveas(sharp_v_prec_4,[FigPath 'sharp_vs_prec4.pdf'])    
...
[ir_max_neq,ir_i_neq] = nanmax(ir_vec_neq(pd_indices_neq).*(precision_vec_neq(pd_indices_neq)<=precision_ceiling));
...
[ir_max_eq,ir_i_eq] = nanmax(ir_vec_eq(pd_indices_eq));
...
scatter(bs_vec,s_max_vec,75,'MarkerFaceColor',cmap_gre(4,:),'MarkerEdgeColor','k');

ylabel('maximum achivable sharpness (H)')
% ylim([2 10])
...
plot(bs_vec,p_max_vec,'-','Color','k','LineWidth',2)
scatter(bs_vec,p_max_vec,75,'s','MarkerFaceColor',cmap_rd(4,:),'MarkerEdgeColor','k');
...
for i = 1:length(master_struct_multi_g)-1
    ...
    if i > 1
        precision_vec = vertcat(exp(metric_array(:,precision_index_num)),metric_array_supp(:,precision_index_num).^2);
    else
    ...
    options = find(sharpness_vec<=s_max&precision_vec<=p_max&ir_vec<=ir_ceil);
    ...
    ir_99 = 0.99*nanmax(ir_vec(options));
    ir_options = find(ir_vec(options)>=ir_99);
%     if length(ir_options) < 10
%         ir_99 = prctile(ir_vec(options),99.7);
%         ir_options = find(ir_vec(options)>=ir_99);
%     end
