...
n_points = 7;
PhiMax = 15;
phi_bins = [0 logspace(-2, log10(PhiMax), n_points) Inf];
...
% n_levels = 7;
i_max = 4;
levels = (0:.25:i_max)/(0.25^2*8);%max(info_array(:));
n_levels = ceil(length(levels)/2);
ir_axis = 0:1:i_max;
ir_labels = round((0:.5:2) * c_factor * log2(exp(1)),3);
% downsample 
sharp_bound_eq = [0 0.0625 0.0625];
prec_bound_eq = [8 8 0];
s_eq = 0.25^2;
p_eq = 8;
...
% get mask for attainable region
% region_mask = poly2mask(sharp_bound_neq, prec_bound_neq, length(precision_vec), length(sharpness_vec)); 

close all
contour_fig = figure;

hold on
cmap1 = flipud(brewermap(length(phi_bins),'Spectral'));
cmap2 = contour_alpha*(brewermap(n_levels+3,'Purples'))+([1 1 1]-contour_alpha);
cmap2 = cmap2(5:end,:);
cmap2 = [cmap2 ; repmat(cmap2(end,:),n_levels,1)];
colormap(cmap2)
[~,ch] = contourf(sharpness_vec/s_eq,precision_vec/p_eq,info_array,levels,'ShowText','on','LineStyle','-');
%%
plot(sharp_bound_neq/s_eq,prec_bound_neq/p_eq,'-','Color','k','LineWidth',2)
plot(sharp_bound_eq/s_eq,prec_bound_eq/p_eq,'--','Color','k','LineWidth',3)
% gray out rest of plot region
f = fillout(sharp_bound_neq/s_eq,prec_bound_neq/p_eq,[0 4 0 2]);
plot(mean(s_array)/s_eq, mean(p_array)/p_eq,'-k','LineWidth',1.5)
for m = 1:length(phi_bins)-1
    scatter(s_array(:,m)/s_eq, p_array(:,m)/p_eq,45,'MarkerFaceColor',cmap1(m+1,:),'MarkerEdgeColor','k',...
              'MarkerFaceAlpha',0.2,'MarkerEdgeAlpha',0.1)            
end

f.FaceColor = [0.5 0.5 0.5];
f.FaceAlpha = 0.5;

xlabel('sharpness gain (dr/dc)^2');
