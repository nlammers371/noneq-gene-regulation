    ...
    for p = 1:size(ir_rate_array_g_neq,3)
        ...
        for i = 1:4
            ...
            for r = 1:size(rates,1)
                ...
                Q_num = RSymFun(paramCell{:});

                ss_full = calculate_ss_num(Q_num,numerical_precision);  

                % get production rate
                pd_temp(r) = sum(ss_full(sweepInfo.activeStateFilter));
                ...
                Q_num_c0 = RSymFun(paramCellC0{:});
                Q_num_c1 = RSymFun(paramCellC1{:});
                ...
                ProductionRate1 = sum(state_probs_c1(:,sweepInfo.activeStateFilter==1),2);
                ProductionRate0 = sum(state_probs_c0(:,sweepInfo.activeStateFilter==1),2);   
...
scatter(cw_vec_neq_bs(ir_keep_indices_bs_neq),sharpness_array_norm_bs_neq(:,m_ind)*m_factor(m_ind),[],ir_keep_values_bs_neq,'filled','o','MarkerEdgeColor','k','MarkerFaceAlpha',0.25,'MarkerEdgeAlpha',0)
scatter(cw_vec_eq_bs(ir_keep_indices_bs_eq),sharpness_array_norm_bs_eq(:,m_ind)*m_factor(m_ind),'s','MarkerFaceColor',cmap(3,:),'MarkerEdgeColor','k')
