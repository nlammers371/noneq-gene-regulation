...
FigPath = 'S:\Nick\Dropbox\Nonequilibrium\Nick\manuscript\experimental_signatures\';
mkdir(FigPath)
...
rate_array_neq0 = NaN(length(cw_vec)*length(rate_axis)*n_keep,11);
...
ir_array_neq0 = NaN(length(cw_vec),length(rate_bins)-1,n_keep);
...
for c = 1:length(cw_vec)    
    ...
    for r = 1:length(rate_bins)-1
        ...
        rate_filter_neq = find(rate_filter_neq1 & ir_vec_neq <= 1*max_ir_neq);
        ...
        neq0_ids = randsample(rate_filter_neq,n_keep,true);
        ir_array_neq0(c,r,:) = reshape(sweep_results_neq.metric_array(neq0_ids,ir_index),1,1,[]);
        ...
        rate_array_neq0(first_i:first_i+n_keep-1,:) = sweep_results_neq.rate_array(neq0_ids,:);
...
% now do this for the equilibrium "hord"
neq_rate_array = repmat(rate_array_neq0, 1,1,length(m_factor));
neq_rate_array(:,sweep_info_neq.unbindingFlags==1,:) = neq_rate_array(:,sweep_info_neq.unbindingFlags==1,:) .* reshape(m_factor,1,1,[]);
neq_rate_array(:,sweep_info_neq.b_index,:) = repmat(reshape(alpha_adjusted,1,1,[]),size(neq_rate_array,1),1,1);
...
sharpness_vec_neq0 = NaN(size(neq_rate_array,1),size(neq_rate_array,3));
...
rate_vec_neq0 = NaN(size(neq_rate_array,1),size(neq_rate_array,3));
...
parfor p = 1:size(ir_rate_array_neq,3)
    ...
    paramCellNeq0 = mat2cell(neq_rates0,size(neq_rates0,1),ones(1,size(neq_rates0,2)));    
    
    % production rate     
    rate_vec_neq(:,p) = productionRateFunction(paramCellNeq{:});    
    rate_vec_eq0(:,p) = productionRateFunction(paramCellEq0{:});  
    rate_vec_neq0(:,p) = productionRateFunction(paramCellNeq0{:});  
    ...
    sharpness_vec_neq0(:,p) = sharpnessFunction(paramCellNeq0{:});% ./ (rate_vec_eq_full(:,p).*(1-rate_vec_eq_full(:,p)));        
...
sharpness_array_norm_neq0 = sharpness_vec_neq0 ./ sharpness_vec_neq0(:,1);

rate_array_norm_neq = rate_vec_neq ./ rate_vec_neq(:,1);
rate_array_norm_eq0 = rate_vec_eq0 ./ rate_vec_eq0(:,1);
rate_array_norm_neq0 = rate_vec_neq0 ./ rate_vec_neq0(:,1);

%% add normalized sharpness (H)
h_array_neq = sharpness_vec_neq ./ (rate_vec_neq.*(1-rate_vec_neq));
h_array_norm_neq = h_array_neq ./ h_array_neq(:,1);
h_array_eq0 = sharpness_vec_eq0 ./ (rate_vec_eq0.*(1-rate_vec_eq0));
h_array_norm_eq0 = h_array_eq0 ./ h_array_eq0(:,1);
h_array_neq0 = sharpness_vec_neq0 ./ (rate_vec_neq0.*(1-rate_vec_neq0));
h_array_norm_neq0 = h_array_neq0 ./ h_array_neq0(:,1);

r_array_neq = rate_vec_neq ./ (rate_vec_neq.*(1-rate_vec_neq));
r_array_norm_neq = r_array_neq ./ r_array_neq(:,1);
r_array_eq0 = rate_vec_eq0 ./ (rate_vec_eq0.*(1-rate_vec_eq0));
r_array_norm_eq0 = r_array_eq0 ./ r_array_eq0(:,1);
r_array_neq0 = rate_vec_neq0 ./ (rate_vec_neq0.*(1-rate_vec_neq0));
r_array_norm_neq0 = r_array_neq0 ./ r_array_neq0(:,1);

%% Save build arrays
plot_data = struct;
plot_data.rate_axis = rate_axis;
plot_data.cw_vec = cw_vec;
plot_data.rate_index_vec = rate_index_vec;
plot_data.rate_index_vec0 = rate_index_vec0;
plot_data.cw_index_vec = cw_index_vec;
plot_data.cw_index_vec0 = cw_index_vec0;

plot_data.sharpness_vec_neq = sharpness_vec_neq;
plot_data.sharpness_array_norm_neq = sharpness_array_norm_neq;
plot_data.sharpness_vec_eq0 = sharpness_vec_eq0;
plot_data.sharpness_array_norm_eq0 = sharpness_array_norm_eq0;
% plot_data.sharpness_vec_neq0 = sharpness_vec_neq0;
% plot_data.sharpness_array_norm_neq0 = sharpness_array_norm_neq0;

plot_data.rate_vec_neq = rate_vec_neq;
plot_data.rate_array_norm_neq = rate_array_norm_neq;
plot_data.rate_vec_eq0 = rate_vec_eq0;
plot_data.rate_array_norm_eq0 = rate_array_norm_eq0;
% plot_data.rate_vec_neq0 = rate_vec_neq0;
% plot_data.rate_array_norm_neq0 = rate_array_norm_neq0;

plot_data.h_array_norm_neq = h_array_norm_neq;
plot_data.h_array_norm_eq0 = h_array_norm_eq0;
% plot_data.h_array_norm_neq0 = h_array_norm_neq0;

plot_data.alpha_factor = alpha_factor;

save([FigPath 'plot_data.mat'],'plot_data')
...
% rate_adjusted_eq0 = r_array_norm_eq0(rate_filter0,m_ind)./(1-rate_axis(rate_index_vec0)'/m_factor(m_ind));
% rate_adjusted_neq = r_array_norm_neq(rate_filter,m_ind)./(1-rate_axis(rate_index_vec)'/m_factor(m_ind));
% 
% close all
% figure;
% hold on
% scatter(rate_adjusted_eq0,h_array_norm_eq0(rate_filter0,m_ind).*m_factor(m_ind),50)
% % scatter(rate_array_norm_neq0(rate_filter0,m_ind),sharpness_array_norm_neq0(rate_filter0,m_ind).*m_factor(m_ind),50,reshape(ir_array_neq0(:,r_ind,:),[],1))
...
% scatter(rate_adjusted_neq, h_array_norm_neq(rate_filter,m_ind).*m_factor(m_ind),50,'filled','s')
...
colormap(cmap2)
hold on
scatter(cw_vec(cw_index_vec0(rate_filter0)),h_array_norm_eq0(rate_filter0,m_ind).*m_factor(m_ind),'s','MarkerFaceColor',cmap(3,:),'MarkerEdgeColor','k')
scatter(cw_vec(cw_index_vec(rate_filter)),imgaussfilt(h_array_norm_neq(rate_filter,m_ind).*m_factor(m_ind),1),markerSize,'MarkerFaceColor',cmap(2,:),'MarkerEdgeColor','k')

set(gca,'xscale','log')
xlabel('relative non-cognate factor concentration (c / c)');
ylabel('fold sharpness change (H /H \times k/k)')
...
close all
n_plot = 25;
m_ind_vec = fliplr([m_ind 5 10]);
cw_vec_plot = cw_vec(1:find(cw_vec<=1e5,1,'last'));
% generate colormaps
c_cell = cell(1,3);
...
for m = 1:length(m_ind_vec)
    m_ind_2 = m_ind_vec(m);
    cmap = c_cell{m};
    r_vec = rate_array_norm_neq(:,m_ind_2);
    s_vec = sharpness_array_norm_neq(:,m_ind_2);
    for c = 1:2:length(cw_vec_plot)
        plot_indices_eq = randsample(find(cw_index_vec0==c&rate_filter0),n_plot);
        scatter(rate_array_norm_eq0(plot_indices_eq,m_ind_2),sharpness_array_norm_eq0(plot_indices_eq,m_ind_2).*m_factor(m_ind_2),[],...
              's','MarkerFaceColor',cmap(c,:),'MarkerEdgeColor','k','MarkerEdgeAlpha',.1)
            
        if c == 53
            s(m) = scatter(imgaussfilt(r_vec(cw_index_vec==c&rate_filter),1),imgaussfilt(s_vec(cw_index_vec==c&rate_filter),1).*m_factor(m_ind_2),15 + (c/2)^2/20,...
                      'MarkerFaceColor',cmap(c,:),'MarkerEdgeColor','k','MarkerEdgeAlpha',.2,'MarkerFaceAlpha',0.75);
        else
            scatter(imgaussfilt(r_vec(cw_index_vec==c&rate_filter),1),imgaussfilt(s_vec(cw_index_vec==c&rate_filter),1).*m_factor(m_ind_2),15 + (c/2)^2/20,...
                      'MarkerFaceColor',cmap(c,:),'MarkerEdgeColor','k','MarkerEdgeAlpha',.2,'MarkerFaceAlpha',0.75);
        end
        
        if mod(c,5)==0 && m == 1
            scatter(.1+c/500,.25,15 + (c/2)^2/20,...
