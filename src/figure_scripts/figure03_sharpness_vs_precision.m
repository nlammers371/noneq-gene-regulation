...
[~,sharp_i_neq] = nanmax(ir_vec_neq(pd_indices_neq).*(precision_vec_neq(pd_indices_neq)<=precision_ceiling));
sharp_rates_neq = rate_array_sym_neq(pd_indices_neq(sharp_i_neq),:);

[~,ir_i_neq] = nanmax(ir_vec_neq(pd_indices_neq));
...
ir_color = brighten(cmap(8,:),-0.5);

induction_plots_sym = figure('Position',[100 100 512 256]);
cmap = brewermap(8,'Set2');
cmap_rd = brewermap(8,'Reds');
...
fill([c_vec' fliplr(c_vec')],[ubneq' fliplr(lbneq')],cmap(8,:),'FaceAlpha',.1,'EdgeAlpha',0.75,'EdgeColor',cmap(8,:));
pneq = plot(c_vec,r_curve_neq,'Color',ir_color,'LineWidth',2);
...
%% make illustrative scatter ploits in neighborhood of c1 and c0
xl = [0.9 1.1];
yl = [0.42 0.58];
% get indices for c1 and c0
[~,c1_i] = min(abs(c_vec-c1));
[~,c0_i] = min(abs(c_vec-c0));

scatter_fig = figure('Position',[100 100 512 256]);
tiledlayout(1,3)
...
% make area plots to indicate error range for each
errorbar([c0 c1], r_curve_eq([c0_i, c1_i]), p_curve_eq([c0_i, c1_i])/sqrt(n_cycles),'.', 'Color','k','CapSize',0);
scatter([c0 c1], r_curve_eq([c0_i, c1_i]),100,'MarkerFaceColor',ceq,'MarkerEdgeColor','k')
box on
% set(gca,'xtick',[0.2 1 5])
xlim(xl)
ylim(yl)
...
set(gcf,'color','w');
set(gca,'xticklabel',[])
% neq_fig = figure('Position',[100 100 256 256]);
...
hold on
% make area plots to indicate error range for each
errorbar([c0 c1], r_curve_neq([c0_i, c1_i]), p_curve_neq([c0_i, c1_i])/sqrt(n_cycles),'.', 'Color','k','CapSize',0);
scatter([c0 c1], r_curve_neq([c0_i, c1_i]),100,'MarkerFaceColor',ir_color,'MarkerEdgeColor','k')
...
set(gca,'Color',[228,221,209]/255) 
grid on
% neq_fig.InvertHardcopy = 'off';
set(gcf,'color','w');

% prec_fig = figure('Position',[100 100 256 256]);
nexttile
hold on
set(gca,'yticklabel',[])
set(gca,'xticklabel',[])
% make area plots to indicate error range for each
errorbar([c0 c1], r_curve_neq_p([c0_i, c1_i]), p_curve_neq_p([c0_i, c1_i])/sqrt(n_cycles),'.', 'Color','k','CapSize',0);
scatter([c0 c1], r_curve_neq_p([c0_i, c1_i]),100,'MarkerFaceColor',cp,'MarkerEdgeColor','k')
...
prec_fig.InvertHardcopy = 'off';
set(gcf,'color','w');

saveas(scatter_fig,[FigPath 'scatter_motif_c1c0.png'])
saveas(scatter_fig,[FigPath 'scatter_motif_c1c0.pdf']) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
